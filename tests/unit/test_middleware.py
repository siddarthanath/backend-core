...
async def body_limit_client():
    async with AsyncClient(
        transport=ASGITransport(app=_make_app_with_body_limit()), base_url="http://test"
    ) as c:
        yield c
...
class TestBodySizeLimitMiddleware:
    ...
    async def test_passes_through_when_under_limit(
        ...
        response = await body_limit_client.post(
            "/echo", content=body, headers={"content-length": str(len(body))}
        )
        assert response.status_code == 200

    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_returns_413_when_over_limit(
        self, body_limit_client: AsyncClient
    ) -> None:
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_returns_400_for_invalid_content_length(
        self, body_limit_client: AsyncClient
    ) -> None:
        response = await body_limit_client.post(
            "/echo", headers={"content-length": "not-a-number"}
        )
